...
export default function PaymentPage() {
  ...
  useEffect(() => {
    const fetchPayment = async () => {
      try {
        const result = await paymentService.getPayment(params.id as string);
        if (result.success) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (verifyResult.success) {
          setTransactionData(verifyResult.data);
          setStep("success");
        } else {
          ...
          <div className="rounded-2xl bg-slate-900/50 backdrop-blur-xl border border-white/10 p-8">
            ...
            <div className="space-y-4 mb-8">
              <button
                ...
                className={`w-full p-6 rounded-xl border-2 transition-all duration-300 text-left ${paymentMethod === "card"
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center ${paymentMethod === "card" ? "bg-amber-500/20" : "bg-slate-700"
                      }`}
                  >
                    <CreditCard
              ...
              <button
                ...
                className={`w-full p-6 rounded-xl border-2 transition-all duration-300 text-left ${paymentMethod === "bank"
                    ? "border-amber-500 bg-amber-500/10"
                    : "border-white/10 bg-slate-800/50 hover:border-white/20"
                  }`}
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center ${paymentMethod === "bank" ? "bg-amber-500/20" : "bg-slate-700"
                      }`}
                  >
                    <Building2
              ...
              <button
                ...
                className={`w-full p-6 rounded-xl border-2 transition-all duration-300 text-left ${paymentMethod === "ussd"
                    ? "border-amber-500 bg-amber-500/10"
                    : "border-white/10 bg-slate-800/50 hover:border-white/20"
                  }`}
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center ${paymentMethod === "ussd" ? "bg-amber-500/20" : "bg-slate-700"
                      }`}
                  >
                    <Smartphone
        ...
        <div className="rounded-2xl bg-slate-900/50 backdrop-blur-xl border border-white/10 p-12">
          <div className="text-center">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-semibold">
                    {formatCurrency(transactionData.payment.amount)}
                  </span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-mono">
                    {transactionData.receipt.receiptNumber}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">Date</span>
                  <span className="text-white">
                    {formatDate(transactionData.payment.paidDate)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-4 justify-center">
              ...
              <Link
                href={`/student/receipts/${transactionData.receipt._id}`}
                className="px-8 py-3 rounded-xl border border-white/10 text-white font-semibold hover:bg-white/5 transition-all duration-300"
              >
                Download Receipt
              </Link>
            </div>
